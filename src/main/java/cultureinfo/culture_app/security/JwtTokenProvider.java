...
import cultureinfo.culture_app.dto.request.TokenRequestDto;
...
public class JwtTokenProvider {
    ...
    private static final long REFRESH_TOKEN_EXPIRE_TIME = 1000 * 60 * 60 * 24 * 7;  // 7일// 1시간
    ...
    public boolean refreshTokenPeriodCheck(String token) {
        ...
        long refresh_nowTime = new Date(now).getTime();

        if (refresh_expiredTime - refresh_nowTime < THREE_DAYS) {
            return true;
        }
        return false;
    }
    ...
    public JwtToken reissueAccessToken(TokenRequestDto req) {
        ...
        JwtToken tokenDto = null;
        if (refreshTokenPeriodCheck(refreshToken)) {
            //Refresh Token의 유효기간이 3일 미만일 경우 전체(Access / Refresh) 재발급
            tokenDto = generateToken(authentication);

        } else {
            //Refresh Token의 유효기간이 3일 이상일 경우 Access Token만 재발급
            tokenDto = generateAccessToken(authentication);
        }
        return tokenDto;
    }
